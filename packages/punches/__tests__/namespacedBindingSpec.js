describe('Namespaced dynamic bindings', function() {
    ...
    try {
      ...
      ko.bindingHandlers['a'] = {
        update: function(element, valueAccessor) {
          ...
          for (var key in value) if (ko.unwrap(value[key])) lastSubKey = key
        }
      }
      testNode.innerHTML =
        "<div data-bind='a.b: true, a.c: myObservable'></div>"
  ...
  it('Should update only the binding that needs it', function() {
    try {
      ...
      ko.bindingHandlers.test = {
        update: function(element, valueAccessor) {
          var value = valueAccessor()
          for (var key in value) if (ko.unwrap(value[key])) updateCounts[key]++
        }
      }
      testNode.innerHTML =
  ...
  it('Should be able to supply event handler using event.type', function() {
    try {
      ...
      testNode.innerHTML =
        "<button data-bind='event.click: function() { clickCalled = true; }'>hey</button>"
  ...
  it('Should be able to set CSS style using style.stylename', function() {
    try {
      ...
      expect(testNode.childNodes[0].style.backgroundColor).toEqualOneOf([
        'red',
        '#ff0000'
  ...
  it('Should be able to set attribute using attr.name', function() {
    try {
      ...
      ko.applyBindings(model, testNode)
      expect(testNode.childNodes[0].getAttribute('someAttrib')).toEqual(
        'initial value'
      )
...
describe('Auto namespaced bindings', function() {
  ...
  it('Should create and call dynamic binding handler for each sub-value in binding', function() {
    try {
      ...
      testNode.innerHTML =
        "<div data-bind='a: {b: true, c: myObservable}'></div>"
      ko.applyBindings({ myObservable: observable }, testNode)
      expect(lastSubKey).toEqual('b')
