...
import '../src/textFilter'
import { filterPreprocessor } from '../src/textFilter'

/**
 * Implementation requires ko in the global scope
 */
// @ts-ignore
global.ko = ko

describe('Text filters preprocessor', () => {
  it('Should convert basic input|filter syntax into function calls', () => {
    expect(
  ...
  it('Should not be confused by | in quotes, or : or || in input', () => {
    expect(filterPreprocessor('input1||(input2?a:b)||"A|B"|filter')).toEqual(
      'ko.filters[\'filter\'](input1||(input2?a:b)||"A|B")'
    )
  })

  it('Should tolerate spaces or newlines in the input', () => {
    expect(filterPreprocessor('input \n| filter\n | filter2')).toEqual(
      "ko.filters['filter2'](ko.filters['filter'](input))"
    )
  })

  it('Should be able to run filter output and apply filters to input', () => {
    try {
      ko.filters.test = (input) => input.toUpperCase()
  ...
  it('Should pass arguments when applying filter', () => {
    try {
      ko.filters.test = (input, length) => input.slice(0, length)
  ...
  // it('Should convert text to uppercase using uppercase filter', () => {
  //   expect(eval(filterPreprocessor('"someText" | uppercase'))).toEqual(
  //     'SOMETEXT'
  //   )
  //   // Should unwrap observable value
  //   expect(
  //     eval(filterPreprocessor('ko.observable("someText") | uppercase'))
  //   ).toEqual('SOMETEXT')
  // })

  // it('Should convert text to lowercase using lowercase filter', () => {
  //   expect(eval(filterPreprocessor('"someText" | lowercase'))).toEqual(
  //     'sometext'
  //   )
  //   // Should unwrap observable value
  //   expect(
  //     eval(filterPreprocessor('ko.observable("someText") | lowercase'))
  //   ).toEqual('sometext')
  // })

  // it('Should convert blank text, empty array, or null to default value using default filter', () => {
  //   // non-blank value is not converted
  //   expect(eval(filterPreprocessor('"someText" | default:"blank"'))).toEqual(
  //     'someText'
  //   )
  ...
  //   expect(eval(filterPreprocessor('undefined | default:"blank"'))).toEqual(
  //     'blank'
  //   )
  //   // a function is not converted
  //   var emptyFunction = function() {}
  //   expect(eval(filterPreprocessor('emptyFunction | default:"blank"'))).toEqual(
  //     emptyFunction
  //   )
  //   // Should unwrap observable value
  //   expect(
  //     eval(filterPreprocessor('ko.observable("someText") | default:"blank"'))
  //   ).toEqual('someText')
  // })

  // it('Should replace found text in input using replace filter', function() {
  //   expect(
  //     eval(filterPreprocessor('"someText" | replace:"some":"any"'))
  //   ).toEqual('anyText')
  //   // Should unwrap observable value
  //   expect(
  //     eval(
  //       filterPreprocessor('ko.observable("someText") | replace:"some":"any"')
  //     )
  //   ).toEqual('anyText')
  // })
  ...
  //   expect(eval(filterPreprocessor('"someText" | fit:7:"!"'))).toEqual(
  //     'someTe!'
  //   )
  //   // Truncates from the left if specified
  //   expect(eval(filterPreprocessor('"someText" | fit:7::"left"'))).toEqual(
  //     '...Text'
  //   )
  //   // Truncates in the middle if specified
  //   expect(eval(filterPreprocessor('"someText" | fit:7::"middle"'))).toEqual(
  //     'so...xt'
  //   )
  //   // Should unwrap observable value
  //   expect(
  //     eval(filterPreprocessor('ko.observable("someText") | fit:8'))
  //   ).toEqual('someText')
  // })
  ...
  //   // Converts object
  //   expect(
  //     eval(filterPreprocessor('{a:true, b:false, c:null} | json'))
...
//     testNode.innerHTML =
//       "<div data-bind='text: input | lowercase | fit:10 | json'></div>"
